...
public class ScoreTrend extends View {
    ...
    private int brokenLineColor = 0xff02bbb7; //数字和月份的颜色
    private int straightLineColor = 0xffe2e2e2;//0xffeaeaea刻度的一些颜色
    private int textNormalColor = 0xff7e7e7e;//折线上面的那些颜色

    private int maxScore = 700;
    private int minScore = 650;

    private int monthCount = 10;
    private int selectMonth = 10;//选中的月份

    private String[] monthText = new String[]{"6月", "7月", "8月", "9月", "10月", "11月", "12月"};
    private int[] score = new int[]{660, 663, 669, 678, 682, 689, 666};
...
/*直线的画笔（刻度）*/
        ...
        straightPaint.setAntiAlias(true);//去锯齿的
        straightPaint.setStyle(Paint.Style.STROKE);// Paint.Style.STROKE为一条线
        straightPaint.setStrokeWidth(brokenLineWith);
        straightPaint.setColor((straightLineColor));
        straightPaint.setStrokeCap(Paint.Cap.ROUND);//设置笔刷的样式 Paint.Cap.Round ,Cap.SQUARE等分别为圆形、方形
/*虚线的画笔*/
...
selectMonth =1;
    ...
    private void drawBrokenLine(Canvas canvas) {
        brokenPath.reset();//path每次用的时候最好调用一下reset()方法，否则会产生意料之外的效果。
        brokenPaint.setColor(brokenLineColor);
        brokenPaint.setStyle(Paint.Style.STROKE);
        if (score.length == 0) {
            return;
